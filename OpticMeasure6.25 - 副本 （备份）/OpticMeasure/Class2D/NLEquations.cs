...
namespace CSharpAlgorithm.Algorithm
{
	...
	public abstract class NLEquations 
	{
		 ...
		 */
        //10.30自己增加的
        protected virtual double Func(double[] x, double[] y, double[] DistwoPoint, double[] CosijValue)
        {
            //y[0] = x[0] * x[0] + x[1] * x[1] - 2 * x[0] * x[1] * CosijValue[0] - DistwoPoint[0] * DistwoPoint[0];
            //y[1] = x[0] * x[0] + x[2] * x[2] - 2 * x[0] * x[2] * CosijValue[1] - DistwoPoint[2] * DistwoPoint[2];
            //y[2] = x[1] * x[1] + x[2] * x[2] - 2 * x[1] * x[2] * CosijValue[2] - DistwoPoint[1] * DistwoPoint[1];
		...
		{ 
			// 初始化矩阵
            MatrixOfAlgorithm mtxQ = new MatrixOfAlgorithm();
			mtxQ.Init(n, n);
		...
		{ 
			...
			while (l>0)
			{
                MatrixOfAlgorithm mtxP = new MatrixOfAlgorithm(m, n);
                MatrixOfAlgorithm mtxD = new MatrixOfAlgorithm(m, 1);
				p = mtxP.GetData();
				d = mtxD.GetData();
				...
				// 临时矩阵
                MatrixOfAlgorithm mtxAP = new MatrixOfAlgorithm();
                MatrixOfAlgorithm mtxU = new MatrixOfAlgorithm();
                MatrixOfAlgorithm mtxV = new MatrixOfAlgorithm();
				// 解矩阵
                MatrixOfAlgorithm mtxDX = new MatrixOfAlgorithm();
				// 基于广义逆的最小二乘解
